...
def get_snapshot_id_by_name(vmid, snapshot_name, credentials):
    ...
    try:
        ...
        for snapshot in snapshots:
            ...
            snap_id = snapshot.get("snapid") or snapshot.get("snapname") or snapshot.get("name")
        ...
        print(f"可用的快照:")
        for snapshot in snapshots:
            print(f"   - {snapshot.get('name')} (ID: {snapshot.get('snapid')}, 描述: {snapshot.get('description', '无')})")
...
def recover_vm_snapshot(vm_name_or_id=None, snapshot_name=None):
    """
    发起恢复虚拟机快照的请求
    基于提供的curl命令实现
    
    Args:
        vm_name_or_id: 虚拟机名称或ID
    ...
    if vm_name_or_id is None and len(sys.argv) > 1:
        ...
        print("请提供虚拟机名称或ID")
        print("用法: python vm_recover.py <虚拟机名或ID> <快照名称>")
        print("示例: python vm_recover.py \"XMG5100_2.127_0001\" \"test_snapshot_20260121\"")
        print("      python vm_recover.py 5370079826048 \"test_snapshot_20260121\"")
        return None
    
    if snapshot_name is None and len(sys.argv) > 2:
    ...
    # 4. 请求URL
    url = f"https://{HCI_DEVICE_IP}/vapi/extjs/cluster/vm/{vmid}/recovery"
...
if __name__ == "__main__":
    ...
    if len(sys.argv) >= 3:
        ...
        recover_vm_snapshot(vm_name_or_id, snapshot_name)
    else:
        # 参数不足，显示用法提示
        ...
        print()
        print("示例:")
        print("  python vm_recover.py \"XMG5100_2.127_0001\" \"test_snapshot_20260121\"")
        print("  python vm_recover.py 5370079826048 \"test_snapshot_20260121\"")
        print()
        print("说明:")
        print("  - 虚拟机名或ID: 支持虚拟机名称（如XMG5100_2.127_0001）或虚拟机ID（如5370079826048）")
        print("  - 快照名称: 要恢复的快照名称")
        print("  - 系统会先从snapshot_mapping.json中查找快照，如果找不到则直接从API查询")
        print("  - 恢复快照会将虚拟机恢复到指定快照的状态")

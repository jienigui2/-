...
def get_vmid_by_name(vm_name, credentials):
    ...
    # 请求 URL：获取集群虚拟机列表
    url = f"https://{HCI_DEVICE_IP}/vapi/extjs/cluster/vms"
...
def get_vm_snapshot_list(vm_name_or_id):
    """
    获取虚拟机的所有快照信息
    ...
    # 3. 获取虚拟机快照列表
    print(f"正在获取虚拟机 {vmid} 的快照列表...")
    
    url = f"https://{HCI_DEVICE_IP}/vapi/extjs/cluster/vm/{vmid}/snapshot"
    ...
    try:
        ...
        for i, snapshot in enumerate(snapshots, 1):
            ...
            if snaptime:
                ...
                snaptime_str = datetime.fromtimestamp(snaptime).strftime("%Y-%m-%d %H:%M:%S")
            else:
                snaptime_str = "未知"
            
            print(f"{i}. 快照名称: {snap_name}")
            print(f"   快照ID: {snap_id}")
            print(f"   描述: {description}")
            print(f"   创建时间: {snaptime_str}")
            print()
...
if __name__ == "__main__":
    ...
    if len(sys.argv) >= 2:
        ...
        get_vm_snapshot_list(vm_name_or_id)
    else:
        # 没有提供参数，显示用法提示
        print("参数不足，请提供虚拟机名称或ID")
        print()
        print("用法: python vm_photo_list.py <虚拟机名或ID>")
        print()
        print("示例:")
        print("  python vm_photo_list.py \"XMG5100_2.127_0001\"")
        print("  python vm_photo_list.py 5370079826048")
        print("  python vm_photo_list.py 8501315397338")
        print()
        print("说明:")
        print("  - 虚拟机名或ID: 支持虚拟机名称（如XMG5100_2.127_0001）或虚拟机ID（如5370079826048）")
        print("  - 系统会获取并显示该虚拟机的所有快照信息（名称、ID、描述、创建时间）")
